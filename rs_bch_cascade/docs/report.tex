...
\item \textbf{时延权威口径}以 MATLAB 最终仿真 (\texttt{rs\_bch\_cascade\_matlab/}) 的 LLOSD 并行结果为准：
     相对纯 RS 软判基线 (LCC-BR)，串行 $P{=}1$ 时延增幅 \textbf{$+18.7\%$ (n=255) / $+26.8\%$ (n=127)}；
     LLOSD 用 Lagrange 插值、无主元、可并行 (时延 $\approx$ ops$/P$)，
     \textbf{$P{=}2$ (n=255) $\to +9.4\%$、$P{=}3$ (n=127) $\to +8.9\%$ 即 $\leq 10\%$ KPI 达标}——
     这是 OSD 串行高斯消元做不到的并行优势
\item 旁证：本 Python 实现在同一软判基线下测得时延\emph{反而减少 $20$--$27\%$}，
     与 MATLAB 口径存在实现差异 (外码 LCC-BR 输入洁净度、Monte Carlo 抽样)，但量级一致 ($\sim 10^5$ F$_2^m$/帧)
\item 相对纯 RS 硬判基线 (BM)，级联译码时延增加约 1200\%，
     同样以并行视角 (时钟周期 $\approx$ ops$/P$) 补充说明
\end{itemize}
\end{keybox}
...
\item \textbf{Case (b) vs Pure RS-LCC-BR 软判基线}:
  本 Python 实现测得 Cascade 时延\textbf{反而减少 20\%--27\%}。这是因为 Cascade A 的 LLOSD 内码
  ($\tau=2$) 只枚举少量 TEP 并使用早期终止，
  而纯 LCC-BR 每个 test-vector 都跑一次 BM。
  \emph{注：项目的时延\textbf{权威口径}以 MATLAB 最终仿真 (\texttt{rs\_bch\_cascade\_matlab/}) 的
  LLOSD 并行结果为准——串行 $P{=}1$ 为 $+18.7\%$ (n=255) / $+26.8\%$ (n=127)，
  无主元并行下 $P{=}2$/$P{=}3$ 即 $\leq 10\%$ 达标；本 Python 的 $-20\%$--$-27\%$ 属不同实现口径的旁证
  (外码输入洁净度、Monte Carlo 抽样)，两者量级一致 ($\sim 10^5$ F$_2^m$/帧)。}
\end{itemize}
\end{resultbox}
...
\item \textbf{时延 (软判基线，权威口径)}: 以 MATLAB 最终仿真的 LLOSD 并行结果为准，
      相对纯 RS-LCC-BR 串行 $P{=}1$ 增幅 $+18.7\%$ (n=255) / $+26.8\%$ (n=127)；
      LLOSD 无主元、可并行 (时延 $\approx$ ops$/P$)，$P{=}2$/$P{=}3$ 即 $\leq 10\%$ 达标——
      OSD 串行高斯消元无法这样并行。
      (本 Python 实现在同一基线测得 $-20\%$--$-27\%$，属实现口径差异的旁证；
      主因是内码 LLOSD 的 ML 早期终止在中高 SNR 几乎``一击命中''，量级仍与 MATLAB 一致。)
...
\item \textbf{Case (b) 权威口径}: 串行 $P{=}1$ 为 $+18.7\%$/$+26.8\%$，
  无主元并行 LLOSD 下 $P{=}2$ (n=255) / $P{=}3$ (n=127) 即达标——OSD 串行 GE 无法这样并行
  (本 Python 实现测得 $-20\%\ldots-27\%$ 为实现口径旁证)
\item \textbf{Case (a)} 需要针对性优化：
  \begin{itemize}[leftmargin=1em]
  \item 用无主元并行 LLOSD 的更高并行度 $P$ 进一步压缩时延
