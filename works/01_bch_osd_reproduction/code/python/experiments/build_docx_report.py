...
from work_paths import WORK_ROOT, FIGURES_DIR
...
# Canonical Work 01 artifact locations (the legacy constants above are kept
# only for textual continuity in the generated report).
PROJECT_DIR = WORK_ROOT
FIG_DIR = FIGURES_DIR
PDF_PATH = PROJECT_DIR / "docs" / "reference_paper" / "Efficient_Ordered_Statistics_Decoding_of_BCH_Codes_Without_Gaussian_Elimination.pdf"
OUT = PROJECT_DIR / "docs" / "复现报告_Efficient_OSD_BCH_Without_GE.docx"
...
add_para(doc, "本 Word 报告嵌入了论文 PDF 原文（下方图标）。请双击图标打开 PDF；如果嵌入失效，可直接访问项目仓库 works/01_bch_osd_reproduction/docs/reference_paper/ 目录，或通过 DOI 链接下载。",
         size=10)
...
try:
    ...
    add_para(doc, "（若 Word 版本不支持内嵌 PDF，请从 GitHub 仓库 works/01_bch_osd_reproduction/docs/reference_paper/ 目录下载）",
