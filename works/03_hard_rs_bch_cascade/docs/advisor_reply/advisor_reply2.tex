...
\begin{actionbox}[title=\textbf{Q8. Python vs Matlab？}]
Lagendijk 论文的时延数字来自硬件综合，但本课题的目标是\textbf{在算法层面验证时延优势}
（用抽象的时钟周期模型 + $\mathbb{F}_{2^m}$ ops 双口径）。请问本次交付：

\begin{itemize}[leftmargin=1em, itemsep=1pt]
\item [(a)] 只做 Python 仿真 + 抽象时延模型 \textbf{(推荐, 与第一阶段一致)}
\item [(b)] 加上 Matlab port (与第一阶段的最终交付格式一致)
\end{itemize}
\end{actionbox}
...
\textbf{第一阶段代码}: \url{https://github.com/a-yeyang/efficient-osd-bch-no-ge/tree/main/works/02_soft_rs_bch_cascade}\\
