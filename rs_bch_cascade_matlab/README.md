...
**「RS+BCH vs 单独 RS 时延 ≤10%」KPI —— 本项目的权威结论（Case b，对软判基线 RS-LCC-BR）：**
本实现实测级联 LLOSD 比纯 RS-LCC-BR 在 P=1 串行直接 op 计数下高
**+18.7%（n=255, 9 dB）/ +26.8%（n=127, 9 dB）**，略高于 10% 理想线。**但 LLOSD 用
Lagrange 插值构造 `G_RS`、无主元依赖、天然可并行**，故其内码增量时延在 P 路并行硬件下
≈ ops/P。据此，**只需极小的并行度即达标：P=2（n=255）→ +9.4%、P=3（n=127）→ +8.9%，
均 ≤10% KPI**（见 `figures/matlab_parallel_kpi.png`，clock-cycle ≈ 总运算/P，F₂ᵐ 口径）。
这正是 LLOSD 胜过 OSD 的一大要点——**OSD 的高斯消元是串行的、无法这样并行**；而本口径也与
论文自身的时延度量一致（论文假设「rows of `G_RS` are generated in parallel」）。

> 旁证（口径差异，非头条）：主论文 Python 实现里 Case b 反而为 −20~−27%，这与本实现存在
> 实现口径差异（外码 LCC-BR 输入洁净度、蒙特卡洛抽样），但**量级完全一致（皆 ~10⁵ F₂ᵐ/帧）**。
> **对硬判基线 RS-BM（Case a），因 BM 极快（~6.5k F₂ᵐ），级联在 P=1 下必然远超 10%，同样靠
> 并行视角（clock-cycle ≈ ops/P）补充**——与论文结论一致。
